    ...
    <div class="min-h-screen bg-white flex flex-col">
        ...
        <div class="w-full max-w-[430px] mx-auto px-[22px] mt-[15px]">
            <div class="bg-[#F5F4F8] rounded-full p-1 flex items-center justify-center gap-[7px]">
                ...
                <button wire:click="setActiveTab('notifications')"
                    class="w-1/2 py-2.5 rounded-full text-[12px] font-semibold font-lato transition-all duration-300 {{ $activeTab === 'notifications' ? 'bg-white text-[#252B5C] shadow-sm' : 'text-[#A1A5C1] hover:text-[#252B5C]' }}">
                    Notifications
                </button>
            </div>
        </div>
        ...
        <div class="w-full max-w-[430px] mx-auto px-6 mt-[20px] pb-10 flex-grow">
            ...
            <div class="animate-fade-in">
                <div class="flex items-center justify-between mb-[15px]">
                    <div class="flex items-center gap-[5px]">
                        <span class="text-lg font-bold font-lato tracking-wider text-[#252B5C]">{{ $reviews->total() }}</span>
                ...
                @if ($reviews->hasPages())
                    <div class="mt-6 flex items-center justify-center gap-2">
                        ...
                        @else
                            <button wire:click="previousPage('reviews')" class="w-9 h-9 rounded-full bg-white border border-[#ECEDF3] flex items-center justify-center text-[#252B5C] hover:bg-[#F9443D] hover:text-white hover:border-[#F9443D] transition-all duration-300">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                            </button>
                        @endif
                        ...
                        @foreach (range(1, $reviews->lastPage()) as $page)
                            @if ($page == $reviews->currentPage())
                                <span class="w-9 h-9 rounded-full bg-[#F9443D] flex items-center justify-center text-white font-semibold text-sm shadow-md">{{ $page }}</span>
                            @else
                                <button wire:click="gotoPage({{ $page }}, 'reviews')" class="w-9 h-9 rounded-full bg-white border border-[#ECEDF3] flex items-center justify-center text-[#252B5C] hover:bg-[#F9443D] hover:text-white hover:border-[#F9443D] transition-all duration-300 font-medium text-sm">{{ $page }}</button>
                            @endif
                        @endforeach

                        @if ($reviews->hasMorePages())
                            <button wire:click="nextPage('reviews')" class="w-9 h-9 rounded-full bg-white border border-[#ECEDF3] flex items-center justify-center text-[#252B5C] hover:bg-[#F9443D] hover:text-white hover:border-[#F9443D] transition-all duration-300">
            ...
            <div class="animate-fade-in">
                <div class="flex items-center justify-between mb-[15px]">
                    <div class="flex items-center gap-[5px]">
                        <span class="text-lg font-bold font-lato tracking-wider text-[#252B5C]">{{ $notifications->total() }}</span>
                        <span class="text-lg font-medium font-lato tracking-wider text-[#252B5C]"> Notifications</span>
                    </div>
                    @if ($notifications->count() > 0)
                        <button wire:click="markAllAsRead" class="text-sm text-blue-600 hover:text-blue-800 transition-colors font-medium">
                            Mark all as read
                        </button>
                    @endif
                </div>
                ...
                <div class="space-y-3">
                    @foreach ($notifications as $notificationRead)
                        <div class="bg-white rounded-[18px] p-4 border border-[#ECEDF3] hover:shadow-lg transition-all duration-300 {{ !$notificationRead->is_read ? 'bg-blue-50 border-blue-200' : '' }}">
                            <div class="flex justify-between items-start">
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-2">
                                        <h4 class="text-sm font-bold font-lato text-[#252B5C] {{ !$notificationRead->is_read ? 'font-bold' : '' }}">
                ...
                @if ($notifications->hasPages())
                    <div class="mt-6 flex items-center justify-center gap-2">
                        ...
                        @else
                            <button wire:click="previousPage('notifications')" class="w-9 h-9 rounded-full bg-white border border-[#ECEDF3] flex items-center justify-center text-[#252B5C] hover:bg-[#F9443D] hover:text-white hover:border-[#F9443D] transition-all duration-300">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                            </button>
                        @endif
                        ...
                        @foreach (range(1, $notifications->lastPage()) as $page)
                            @if ($page == $notifications->currentPage())
                                <span class="w-9 h-9 rounded-full bg-[#F9443D] flex items-center justify-center text-white font-semibold text-sm shadow-md">{{ $page }}</span>
                            @else
                                <button wire:click="gotoPage({{ $page }}, 'notifications')" class="w-9 h-9 rounded-full bg-white border border-[#ECEDF3] flex items-center justify-center text-[#252B5C] hover:bg-[#F9443D] hover:text-white hover:border-[#F9443D] transition-all duration-300 font-medium text-sm">{{ $page }}</button>
                            @endif
                        @endforeach

                        @if ($notifications->hasMorePages())
                            <button wire:click="nextPage('notifications')" class="w-9 h-9 rounded-full bg-white border border-[#ECEDF3] flex items-center justify-center text-[#252B5C] hover:bg-[#F9443D] hover:text-white hover:border-[#F9443D] transition-all duration-300">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                            </button>
                        @else
                            <span class="w-9 h-9 rounded-full bg-[#F5F4F8] flex items-center justify-center text-[#A1A5C1] cursor-not-allowed">
