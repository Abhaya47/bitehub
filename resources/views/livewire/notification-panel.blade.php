        ...
        <div class="max-h-80 overflow-y-auto">
            @if(count($notifications) > 0)
                @foreach($notifications as $notificationRead)
                    <div class="px-4 py-3 hover:bg-gray-50 border-b border-gray-100 {{ !$notificationRead->is_read ? 'bg-blue-50' : '' }} transition-colors">
                        <div class="flex justify-between items-start">
                            <div class="flex-1">
                                <div class="flex items-center space-x-2">
                                    <h4 class="font-medium text-gray-900 {{ !$notificationRead->is_read ? 'font-semibold' : '' }}">
                                ...
                                <p class="text-sm text-gray-600 mt-1">{{ Str::limit($notificationRead->notice->message, 100) }}</p>
                                <p class="text-xs text-gray-400 mt-2">{{ $notificationRead->created_at->diffForHumans() }}</p>
                            </div>
                            <div class="flex space-x-1 ml-2">
                                @if(!$notificationRead->is_read)
                                    <button 
                                        wire:click="markAsRead({{ $notificationRead->id }})"
                                ...
                                @endif
                                <button 
                                    wire:click="deleteNotification({{ $notificationRead->id }})"
